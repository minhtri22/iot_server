...
public class CustomerController extends BaseController {
    ...
    public Customer saveCustomer(@io.swagger.v3.oas.annotations.parameters.RequestBody(description = "A JSON value representing the customer.") @RequestBody Customer customer) throws Exception {
        ...
        //Trí: Khi đăng ký mới thì chưa login nên không có tenantId
        //Todo: Kiểm tra người dùng đã login hay chưa
    ...
    //    else 
    //    {
    //     log.debug("saveCustomer log2: {}",getTenantId());
    ...
    // @PostMapping("/customer/public")
    // @ResponseBody
    // public Customer saveCustomerAutoTenant(@RequestBody Customer customer) throws ThingsboardException {
    //     // Lấy tenant đầu tiên từ database (paging = 1 item)
    //     PageData<Tenant> tenants = tenantService.findTenants(new PageLink(1));
    //     if (tenants == null || tenants.getData().isEmpty()) {
...
//Trí: Thêm api endpoint lấy toàn bộ khách hàng theo tenant id
//Mục đích: Tạo endpoint để gán tự động dashboard cho người dùng là customer trong tenant
// @PreAuthorize("hasAuthority('TENANT_ADMIN')")
//     @GetMapping("/tenant/customers/all")
//     public List<Customer> getAllCustomers() throws ThingsboardException {
//         try {
//             TenantId tenantId = getCurrentUser().getTenantId();
//             List<Customer> customers = new ArrayList<>();
//             PageLink pageLink = new PageLink(100); // Load 100 mỗi lần để tránh quá tải
